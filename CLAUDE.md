...
**Beads-only changes always commit and push directly to `main`**, even while
...
feature branch when tickets change, commit `.beads/` to main (stash or
...
- Tests and formatting gate merges: `uv run black --check src tests` and
  `uv run pytest` (CI runs both; main is branch-protected on the `check` job).
