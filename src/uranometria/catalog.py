...
def fmt_coord(ra, dec):
    rh = ra / 15.0
    h = int(rh); m = int(round((rh - h) * 60))
    if m == 60: h, m = h + 1, 0
    sign = "+" if dec >= 0 else "−"
    ad = abs(dec); d = int(ad); am = int(round((ad - d) * 60))
    if am == 60: d, am = d + 1, 0
    return f"{h:02d}h {m:02d}m  {sign}{d:02d}° {am:02d}′"

# ---------------------------------------------------------------- catalogs
TYPE_NAMES = {
    "OCl": "Open cluster", "GCl": "Globular cluster", "G": "Galaxy",
    "PN": "Planetary nebula", "SNR": "Supernova remnant", "EmN": "Emission nebula",
    "RfN": "Reflection nebula", "HII": "H II region", "Neb": "Nebula",
    "Cl+N": "Cluster + nebula", "DrkN": "Dark nebula", "*Ass": "Stellar association",
    "**": "Double star", "*": "Star", "GPair": "Galaxy pair", "GTrpl": "Galaxy triplet",
    "GGroup": "Galaxy group", "Nova": "Nova", "Other": "Deep-sky object",
}
    ...
    def _load_openngc(self):
        ...
        for r in rows:
            if r["Type"] in ("Dup", "NonEx") or not r["RA"]:
                ...
                if r["Type"] == "Dup":
                    tgt = ("NGC" + r["NGC"].zfill(4) if r.get("NGC") else
                           "IC" + r["IC"].zfill(4) if r.get("IC") else None)
            ...
            rec = self._rec(self._display(r["Name"]), ra, dec,
                            TYPE_NAMES.get(r["Type"], r["Type"]),
                            CONST_NAMES.get(r["Const"], r["Const"]), common)
    ...
    def _load_sharpless(self):
        ...
        for line in data_text("sh2.tsv").splitlines():
            ...
            self.by_key[f"SH2{n}"] = self._rec(
                f"Sh2-{n}", float(parts[1]), float(parts[2]),
                "Emission nebula (H II)", "", "")
...
def sesame(desig, timeout=15):
    ...
    if not m:
        return None
    return dict(disp=desig, ra=float(m.group(1)), dec=float(m.group(2)),
                type="Deep-sky object", constellation="", common="")
